  ...
  return (
    <div className="container-custom py-8 md:py-12">
      <div className="mb-8 text-center md:text-start">
        <h1 className="text-2xl md:text-3xl font-display text-blush-400">
          {currentCategory
        ...
        <aside className="hidden md:block w-64 shrink-0">
          <div className="sticky top-24 space-y-8">
            <div className="bg-white rounded-3xl p-5 shadow-soft">
              <h3 className="text-xs font-semibold text-blush-400 uppercase tracking-widest mb-4">
                {t(locale, "categories")}
              </h3>
                ...
                {categories.map((cat) => (
                  <li key={cat.slug}>
                    <button
                      ...
                      className={`w-full text-start text-sm py-2 px-3 rounded-xl transition-all duration-200 ${
                        currentCategory === cat.slug
                          ? "text-white font-medium bg-blush-400 shadow-soft"
                          : "text-charcoal-500 hover:text-blush-400 hover:bg-rose-100"
                      }`}
                    >
                      {cat.name}
                    </button>
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-6 pb-4 border-b border-cream-300">
            ...
            <select
              ...
              className="input-field text-sm py-2.5 w-auto max-w-[200px] rounded-xl cursor-pointer"
            >
              {sortOptions.map((opt) => (
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl shadow-soft">
              <svg className="w-16 h-16 text-rose-200 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-charcoal-400">{t(locale, "noProducts")}</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            ...
            <div className="flex items-center justify-center gap-2 mt-10">
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`w-11 h-11 text-sm font-medium rounded-full transition-all duration-300 ${
                    currentPage === i + 1
                      ? "bg-blush-400 text-white shadow-soft-md"
                      : "bg-white text-charcoal-500 hover:bg-rose-100 hover:text-blush-400 border border-cream-300 shadow-sm"
                  }`}
                >
                  {i + 1}
                </button>
      ...
      {mobileFilterOpen && (
        <>
          <div
            className="fixed inset-0 bg-blush-400/15 backdrop-blur-sm z-40"
            onClick={() => setMobileFilterOpen(false)}
          />
          <div className="fixed inset-y-0 start-0 w-80 max-w-[85vw] bg-warm-white z-50 shadow-soft-xl p-6 overflow-y-auto rounded-r-3xl">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-lg font-display text-blush-400">{t(locale, "filterBy")}</h2>
              <button
                onClick={() => setMobileFilterOpen(false)}
            ...
            <div className="space-y-8">
              <div className="bg-white rounded-2xl p-5 shadow-soft">
                <h3 className="text-xs font-semibold text-blush-400 uppercase tracking-widest mb-3">
                  {t(locale, "categories")}
                </h3>
                <ul className="space-y-1">
                  {categories.map((cat) => (
                    <li key={cat.slug}>
                      <button
                        ...
                        className={`w-full text-start text-sm py-2 px-3 rounded-xl transition-all duration-200 ${
                          currentCategory === cat.slug
                            ? "bg-blush-400 text-white font-medium shadow-soft"
                            : "text-charcoal-500 hover:bg-rose-100 hover:text-blush-400"
                        }`}
                      >
                        {cat.name}
                      </button>
